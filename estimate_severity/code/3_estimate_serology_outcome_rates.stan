...
transformed parameters{
  ...
  // Exponentiate the linear component, to fit to the data
  //outcomeRate = exp(locationIntercept[location] + locationSlope[location] .* ageVec);
  outcomeRate = inv_logit(locationIntercept[location] + locationSlope[location] .* ageVec);
}

model {
  ageSlope ~ normal(0, 0.1);
  //ageSlope ~ normal(5, 3);
  ageSlopeSigma ~ exponential(0.1);
